...
class PurchaseBulkTagging(models.TransientModel):
    _name = 'product.product.bulk.tagging'
    _description = 'Wizard: Add or Remove Tags in bulk'

    x_add_tags = fields.Many2many('custom.tags',
                                  relation='product_product_bulk_tagging_custom_tags_rel1',
                                  column1='product_product_bulk_tagging_id',
                                  column2='custom_tags_id',
                                  string='Add Tags'
                                  )

    x_remove_tags = fields.Many2many('custom.tags',
                                     relation='product_product_bulk_tagging_custom_tags_rel2',
                                     column1='product_product_bulk_tagging_id',
    ...
    def action_confirm(self):
        records = self.env['product.product'].browse(self._context.get('active_ids'))
