...
class HrPayslip(models.Model):
    ...
    def action_payslip_draft(self):
        ...
        moves.filtered(lambda x: x.state in ['cancel', 'posted']).button_draft()
    ...
    def unlink(self):
        if any(self.filtered(lambda payslip: payslip.move_id)):
            raise UserError(_('You cannot delete a payslip which has been posted once.'))
        elif any(self.filtered(lambda payslip: payslip.state not in ('draft', 'cancel'))):
        ...
        for contract in contracts.filtered(lambda contract: contract.resource_calendar_id):
            ...
            for day, hours, leave in day_leave_intervals:
                ...
                for record in holiday:
                    current_leave_struct = leaves.setdefault(record.holiday_status_id, {
                        'name': record.holiday_status_id.name or _('Global Leaves'),
                        'sequence': 5,
                        'code': record.holiday_status_id.name or 'GLOBAL',
                        'number_of_days': 0.0,
                        'number_of_hours': 0.0,
                        'contract_id': contract.id,
                    })
                if not holiday:
                    current_leave_struct = leaves.setdefault(False, {
                        'name': leave.name,
                        'sequence': 5,
                        'code': 'GLOBAL',
                        'number_of_days': 0.0,
...
class HrPayslipRun(models.Model):
    ...
    date_end = fields.Date(string='Date To', required=True, readonly=True, tracking=True,
                           states={'draft': [('readonly', False)]},
                           default=lambda self: fields.Date.to_string(
                               (datetime.now() + relativedelta(months=0, day=1, days=-1)).date()
                           ), )
    credit_note = fields.Boolean(string='Credit Note', readonly=True, tracking=True,
                                 states={'draft': [('readonly', False)]},
    ...
    def unlink(self):
        if any(self.filtered(lambda batch: batch.state not in ('draft', 'close'))):
            raise UserError(_('You cannot delete a payslip batch which is not draft or cancelled!'))
        elif any(self.slip_ids.filtered(lambda payslip: payslip.move_id)):
            raise UserError(_('You cannot delete a payslip which has been posted once.'))
        elif any(self.slip_ids.filtered(lambda payslip: payslip.state not in ('draft', 'cancel'))):
            raise UserError(_('You cannot delete a payslip which is not draft or cancelled!'))
