...
class StockBulkTagging(models.TransientModel):
    _name = 'stock.bulk.tagging'
    _description = 'Wizard: Add or Remove Tags in bulk'

    x_add_tags = fields.Many2many('custom.tags',
                                  relation='stock_bulk_tagging_custom_tags_rel1',
                                  column1='stock_bulk_tagging_id',
                                  column2='custom_tag_id',
                                  string='Add Tags'
                                  )

    x_remove_tags = fields.Many2many('custom.tags',
                                     relation='stock_bulk_tagging_custom_tags_rel2',
                                     column1='stock_bulk_tagging_id',
                                     column2='custom_tag_id',
                                     string='Remove Tags'
                                     )
    ...
    def action_confirm(self):
        records = self.env['stock.landed.cost'].browse(self._context.get('active_ids'))
