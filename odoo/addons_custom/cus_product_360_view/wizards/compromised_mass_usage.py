...
class CompromisedMassUsageEditingWizard(models.TransientModel):
    _name = "compromised.mass.usage.editing.wizard"
    _description = "Compromised Usage Mass Editing"
    ...
    def default_get(self, fields_list):
        res = super(CompromisedMassUsageEditingWizard, self).default_get(fields_list)
    ...
    def filter_component_usage(self):
        for rec in self:
            ...
            usage_ids = rec.x_taraz_part_id.x_usage_ids.filtered(lambda l: l.x_usage_type == 'compromised')
            ...
            for line in rec.x_usage_ids:
                if line.x_line_select:
                    ...
                    line_ids = self.env['component.usage'].search([
                        ...
                        ('x_compromised_id', 'in', rec.x_taraz_part_id.x_compromised_ids.ids),
    ...
    def return_wizard_action(self):
        ...
        form_view = [(self.env.ref('cus_product_360_view.view_compromised_mass_usage_editing_wizard_form').id, 'form')]
        if 'views' in action:
