...
class AccountBulkTagging(models.TransientModel):
    _name = 'hr.contract.bulk.tagging'
    _description = 'Wizard: Add or Remove Tags in bulk'

    x_add_tags = fields.Many2many('custom.tags',
                                  relation='hr_contract_bulk_tagging_custom_tags_rel1',
                                  column1='hr_contract_bulk_tagging_id',
                                  column2='custom_tags_id',
                                  string='Add Tags'
                                  )

    x_remove_tags = fields.Many2many('custom.tags',
                                     relation='hr_contract_bulk_tagging_custom_tags_rel2',
                                     column1='hr_contract_bulk_tagging_id',
    ...
    def action_confirm(self):
        records = self.env['hr.contract'].browse(self._context.get('active_ids'))
